...
module.exports = class PushState {
  ...
  apply (compiler) {
    ...
    compiler.plugin('make', (compilation, done) => {
      const htmlFiles = compiler.options.spike.files.html
      ...
      matchingFiles.forEach((f) => {
        const srcPath = new File(util.conf.context, f).relative
        const outputPath = util.getOutputPath(f).relative.replace(/(.*)?(\..+?$)/, '$1')
        mod += `exports['${outputPath}'] = require('../${srcPath}')\n`
      })
